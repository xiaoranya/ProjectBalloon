...
describe('clarification routing', () => {
  it('adds a team-only contest clarification route', () => {
    ...
    expect(contestant?.children?.find((child) => child.name === 'contest-clarifications')).toMatchObject({
      path: 'clarifications',
    });
  });
  ...
  it('retains clarification and adds printing to the team contest route', () => {
    const contestant = routes.find((route) => route.path === '/contests/:contestId');
    expect(contestant?.children?.find((child) => child.name === 'contest-clarifications')).toMatchObject({ path: 'clarifications' });
    expect(contestant?.children?.find((child) => child.name === 'contest-printing')).toMatchObject({ path: 'printing' });
  });

  it('adds the judge workspace only behind the JUDGE role guard', () => {
    const judge = routes.find((route) => route.path === '/judge');
    expect(judge?.meta).toMatchObject({ requiresAuth: true, requiresStaff: true, requiresJudge: true });
  ...
  it('adds the printer workspace only behind the PRINTER role guard', () => {
    ...
    expect(printer?.meta).toMatchObject({ requiresAuth: true, requiresStaff: true, requiresPrinter: true });
  ...
  it('adds the balloon workspace behind the BALLOON_STAFF role guard', () => {
    ...
    expect(balloon?.meta).toMatchObject({ requiresAuth: true, requiresStaff: true, requiresBalloonStaff: true });
  ...
  it('guards Resolver controls while keeping the official display public', () => {
    ...
    expect(resolver?.meta).toMatchObject({ requiresAuth: true, requiresStaff: true, requiresResolverOperator: true });
    expect(resolver?.children?.find((child) => child.name === 'resolver-home')).toBeDefined();
    expect(routes.find((route) => route.name === 'resolver-display')?.meta?.requiresAuth).toBeUndefined();
  });

  it('guards the awards workspace for award operators', () => {
    const awards = routes.find((route) => route.path === '/awards');
    expect(awards?.meta).toMatchObject({ requiresAuth: true, requiresStaff: true, requiresAwardOperator: true });
    ...
    expect(routes.find((route) => route.name === 'awards-display')?.meta?.requiresAuth).toBeUndefined();
  });

  it('guards screen controls while keeping the registered screen client public', () => {
    expect(routes.find((route) => route.name === 'screen-manage')?.meta).toMatchObject({ requiresAuth: true, requiresScreenOperator: true });
    expect(routes.find((route) => route.name === 'screen-client')?.meta?.requiresAuth).toBeUndefined();
  });
});
