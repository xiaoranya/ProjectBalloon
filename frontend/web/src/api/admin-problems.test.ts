...
const problem = {
  ...
  outputLimitKb: 65536, languages: ['cpp'], testdataVersion: 0, testdataSha256: null,
...
describe('Rust problem administration API contract', () => {
  ...
  it('sends required expectedVersion in the PATCH body', async () => {
    ...
    expect(vi.mocked(fetch).mock.calls[1][1]).toEqual(expect.objectContaining({
      method: 'PATCH', body: JSON.stringify({ expectedVersion: 3, title: 'Three Sum' }),
    }));
  });
  ...
  it('uses exact attachment multipart fields and refreshes the Problem afterward', async () => {
    ...
    expect(vi.mocked(fetch).mock.calls.map(([url]) => url)).toEqual([
      '/api/auth/csrf', '/api/problems/7/attachments', '/api/problems/7',
    ]);
  ...
  it('uses only the file multipart field for test data and refreshes after the incomplete response', async () => {
    vi.mocked(fetch)
      .mockResolvedValueOnce(jsonResponse({ headerName: 'X-XSRF-TOKEN', parameterName: '_csrf', token: 'token' }))
      .mockResolvedValueOnce(jsonResponse({ problemId: 7, version: 1, caseCount: 2, bytes: 20, sha256: 'abc', createdAt: 'now' }, 201))
  ...
  it('refreshes the optimistic concurrency version after statement upsert and attachment delete', async () => {
    ...
    expect(vi.mocked(fetch).mock.calls.map(([url]) => url)).toEqual([
      '/api/auth/csrf', '/api/problems/7/statements/zh-CN', '/api/problems/7',
      '/api/problems/7/attachments/11', '/api/problems/7',
    ]);
  ...
  it('lists, downloads and activates immutable test-data versions', async () => {
    ...
    expect(vi.mocked(fetch).mock.calls[3][1]).toEqual(expect.objectContaining({
      method: 'POST', body: JSON.stringify({ expectedCurrentVersion: 2 }),
    }));
