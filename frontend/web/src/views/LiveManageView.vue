<template>
  <main class="live-manage">
    ...
    <ElCard v-if="contestId" shadow="never">
      ...
      <ElForm label-position="top"><div class="grid"><ElFormItem label="发布状态"><ElSwitch v-model="form.enabled" active-text="已发布" /></ElFormItem><ElFormItem label="主标题"><ElInput v-model="form.title" maxlength="160" /></ElFormItem><ElFormItem label="副标题"><ElInput v-model="form.subtitle" maxlength="240" /></ElFormItem><ElFormItem label="强调色"><ElInput v-model="form.accentColor" maxlength="7" /></ElFormItem><ElFormItem label="榜单行数"><ElInputNumber v-model="form.rowLimit" :min="5" :max="30" /></ElFormItem><ElFormItem label="公告间隔"><ElInputNumber v-model="form.announcementIntervalSeconds" :min="5" :max="60" /></ElFormItem><ElFormItem label="视觉模板"><ElSelect v-model="form.template"><ElOption label="默认" value="DEFAULT" /><ElOption label="电影感" value="CINEMATIC" /><ElOption label="极简" value="MINIMAL" /><ElOption label="分栏" value="SPLIT" /></ElSelect></ElFormItem></div><ElCheckbox v-model="form.showAnnouncements">展示公告</ElCheckbox></ElForm>
    </ElCard>
    <ElCard v-if="contestId" shadow="never">
...
const form = reactive({ enabled: false, title: '', subtitle: '', accentColor: '#22c55e', rowLimit: 12, showAnnouncements: true, announcementIntervalSeconds: 10, template: 'DEFAULT' as 'DEFAULT' | 'CINEMATIC' | 'MINIMAL' | 'SPLIT' });
...
async function createToken() { if (!contestId.value || !label.value.trim()) { ElMessage.warning('请填写用途'); return; } try { const value = await presentationApi.createToken(contestId.value, { label: label.value.trim(), expiresAt: new Date(expiresAt.value).toISOString() }); const fragment = new URLSearchParams({ token: value.token }).toString(); links.value = [['主榜', '/live'], ['首杀', '/live/first-blood'], ['气球', '/live/balloons'], ['封榜倒计时', '/live/freeze-countdown'], ['赛事统计', '/live/statistics']].map(([name, path]) => ({ label: name, url: `${location.origin}${path}?contestId=${contestId.value}#${fragment}` })); label.value = ''; await loadTokens(); } catch (error) { ElMessage.error(getErrorMessage(error)); } }
