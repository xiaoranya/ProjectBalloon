...
const contests = ref<Contest[]>([]); const contestId = ref<number | null>(null); const presentation = ref<AwardPresentation | null>(null);
const loading = ref(false); const saving = ref(false); const errorMessage = ref(''); let generation = 0; let realtime: ContestRealtimeSubscription | undefined;
const draft = reactive({ currentCategoryId: null as number | null, status: 'WAITING' as AwardPresentationStatus, autoRotate: false, intervalSeconds: 15 });
const currentIndex = computed(() => presentation.value?.categories.findIndex((item) => item.id === draft.currentCategoryId) ?? -1);
const statusLabel = computed(() => ({ WAITING: '等待开始', PRESENTING: '进行中', COMPLETED: '已结束' })[draft.status]);

function apply(value: AwardPresentation) { presentation.value = value; draft.currentCategoryId = value.currentCategoryId; draft.status = value.status; draft.autoRotate = value.autoRotate; draft.intervalSeconds = value.intervalSeconds; }
async function load() { if (!contestId.value) return; const id = contestId.value; const token = ++generation; loading.value = true; try { const value = await awardsApi.presentation(id); if (token === generation && id === contestId.value) { apply(value); errorMessage.value = ''; } } catch (error) { if (token === generation) { presentation.value = null; errorMessage.value = getErrorMessage(error); } } finally { if (token === generation) loading.value = false; } }
function connect() { realtime?.stop(); if (!contestId.value) return; realtime = subscribeContestEvents({ contestId: contestId.value, scope: 'PUBLIC', eventTypes: ['AWARDS_UPDATED'], onEvent: () => void load(), poll: load }); }
async function changeContest() { generation += 1; presentation.value = null; await router.replace({ query: contestId.value ? { contestId: String(contestId.value) } : {} }); connect(); await load(); }
async function save() { if (!contestId.value) return; saving.value = true; try { apply(await awardsApi.updatePresentation(contestId.value, { ...draft })); errorMessage.value = ''; ElMessage.success('颁奖展示已更新'); } catch (error) { ElMessage.error(getErrorMessage(error)); await load(); } finally { saving.value = false; } }
async function setStatus(status: AwardPresentationStatus) { draft.status = status; await save(); }
async function move(offset: number) { const category = presentation.value?.categories[currentIndex.value + offset]; if (category) { draft.currentCategoryId = category.id; await save(); } }
function openDisplay() { if (contestId.value) window.open(`/awards/display?contestId=${contestId.value}`, '_blank', 'noopener'); }
