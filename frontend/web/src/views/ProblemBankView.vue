...
const selected = ref<BankProblem>();
async function load() {
  loading.value = true;
  try {
    page.value = await trainingApi.problemBank(
      currentPage.value - 1,
      page.value.size,
      tag.value || undefined,
      difficulty.value,
...
async function open(row: BankProblem) {
  try {
    selected.value = await trainingApi.problem(row.slug);
    drawer.value = true;
  } catch (e) {
    ElMessage.error(getErrorMessage(e));
  }
