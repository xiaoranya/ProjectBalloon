<template>
  <main class="bank-page">
    ...
    <div class="filters"><ElInput v-model="tag" clearable placeholder="标签" @change="load"/><ElSelect v-model="difficulty" clearable placeholder="难度" @change="load"><ElOption v-for="value in 11" :key="value-1" :label="value-1" :value="value-1"/></ElSelect><ElButton :loading="loading" @click="load">刷新</ElButton></div>
...
const page=ref({content:[] as BankProblem[],page:0,size:50,totalElements:0,totalPages:0}); const tag=ref(''); const difficulty=ref<number>(); const loading=ref(false); const drawer=ref(false); const selected=ref<BankProblem>();
async function load(){loading.value=true;try{page.value=await trainingApi.problemBank(0,50,tag.value||undefined,difficulty.value)}catch(e){ElMessage.error(getErrorMessage(e))}finally{loading.value=false}}
