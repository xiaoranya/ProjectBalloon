...
vi.mock('../api/resolver', async (importOriginal) => {
  ...
  return { ...actual, resolverApi: { list: vi.fn(), sources: vi.fn(), create: vi.fn(), get: vi.fn(), publicState: vi.fn(), events: vi.fn(), start: vi.fn(), next: vi.fn(), previous: vi.fn(), pause: vi.fn(), resume: vi.fn(), complete: vi.fn(), autoPlay: vi.fn() } };
});
vi.mock('../realtime/contest-events', () => ({ subscribeContestEvents: vi.fn(() => ({ stop: vi.fn() })) }));

const run = {
  id: 9, contestId: 7, official: true, status: 'RUNNING' as const, currentStep: 0, totalSteps: 1,
...
describe('Resolver views', () => {
  beforeEach(() => {
    ...
    vi.mocked(contestApi.getContest).mockResolvedValue({ id: 7, name: 'Contest 7' } as Awaited<ReturnType<typeof contestApi.getContest>>);
    vi.mocked(resolverApi.list).mockResolvedValue([run]);
    vi.mocked(resolverApi.sources).mockResolvedValue({ publicSnapshot: { id: 11, version: 2, generatedAt: '', payloadSha256: 'a' }, finalSnapshot: { id: 12, version: 3, generatedAt: '', payloadSha256: 'b' } });
  ...
  it('loads only the public run state and subscribes to public Resolver events', async () => {
    ...
    expect(subscribeContestEvents).toHaveBeenCalledWith(expect.objectContaining({ contestId: 7, scope: 'PUBLIC', eventTypes: ['RESOLVER_STATE_CHANGED'] }));
    wrapper.unmount();
  });
});
