...
const sets = [
  { id: 7, slug: 'graphs', title: '图论基础', description: '基础图算法', visibility: 'PUBLIC', itemCount: 2 },
  { id: 8, slug: 'dp', title: '动态规划', description: '状态转移', visibility: 'PUBLIC', itemCount: 1 },
] as const;
...
const details = {
  7: {
    setInfo: sets[0],
    items: [
      { problemId: 1, slug: 'bfs', title: 'BFS', position: 1, required: true, difficulty: 2, tags: ['graph'] },
      { problemId: 2, slug: 'dfs', title: 'DFS', position: 2, required: false, difficulty: null, tags: [] },
    ],
  },
  8: {
    setInfo: sets[1],
    items: [{ problemId: 3, slug: 'knapsack', title: 'Knapsack', position: 1, required: true, difficulty: 4, tags: ['dp'] }],
  },
};
...
describe('TrainingView', () => {
  ...
  it('enrolls in the currently selected training set', async () => {
    ...
    const enrollButton = wrapper.findAll('button').find((button) => button.text().includes('加入训练'));
