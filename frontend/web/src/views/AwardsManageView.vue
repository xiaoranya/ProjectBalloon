...
import { computed, onMounted, reactive, ref } from 'vue'; import { ElMessage, ElMessageBox } from 'element-plus'; import { Download, Lock, MagicStick, Plus, Refresh, Unlock } from '@element-plus/icons-vue'; import { useRoute, useRouter } from 'vue-router'; import { awardsApi, type AwardCategory, type AwardCategoryPayload, type AwardRecipient, type AwardResolverRun, type AwardSet } from '../api/awards'; import { ApiError, getErrorMessage } from '../api/client'; import { contestApi } from '../api/contest'; import type { Contest } from '../api/types'; import { formatDateTime } from '../utils/format';
const route = useRoute(); const router = useRouter(); const contests = ref<Contest[]>([]); const contestId = ref<number | null>(null); const categories = ref<AwardCategory[]>([]); const awardSet = ref<AwardSet | null>(null); const completedRuns = ref<AwardResolverRun[]>([]); const candidates = ref<Awaited<ReturnType<typeof awardsApi.candidates>>>([]); const resolverRunId = ref<number | null>(null); const loading = ref(false); const mutating = ref(false); const exporting = ref(false); const errorMessage = ref(''); const categoryVisible = ref(false); const editing = ref<AwardCategory | null>(null); const manualVisible = ref(false); const manualCategoryId = ref<number | null>(null); const manualTeamId = ref<number | null>(null);
const categoryForm = reactive({ code: '', name: '', displayOrder: 1, includeStar: false, groupName: '', participationType: null as AwardCategory['participationType'], firstBlood: false, ruleType: 'FIXED_COUNT' as AwardCategory['ruleType'], ratio: 0.1, fixedCount: 1, rankFrom: 1, rankTo: 1 });
let generation = 0;
const uniqueTeams = computed(() => new Set(awardSet.value?.recipients.map((item) => item.teamId) ?? []).size); const availableCandidates = computed(() => candidates.value.filter((candidate) => !awardSet.value?.recipients.some((recipient) => recipient.categoryId === manualCategoryId.value && recipient.teamId === candidate.teamId)));
function recipientCount(id: number) { return awardSet.value?.recipients.filter((item) => item.categoryId === id).length ?? 0; } function ruleLabel(category: AwardCategory) { if (category.firstBlood) return '所有 First Blood 队伍'; if (category.ruleType === 'FIXED_COUNT') return `前 ${category.fixedCount} 支符合条件的队伍`; if (category.ruleType === 'RATIO') return `前 ${Math.round((category.ratio ?? 0) * 100)}%`; return `第 ${category.rankFrom}–${category.rankTo} 名`; }
async function loadWorkspace() { if (!contestId.value) return; const activeContest = contestId.value; const request = ++generation; loading.value = true; try { const [nextCategories, runs] = await Promise.all([awardsApi.listCategories(activeContest), awardsApi.completedRuns(activeContest)]); if (request !== generation || activeContest !== contestId.value) return; categories.value = nextCategories; completedRuns.value = runs; resolverRunId.value = awardSet.value?.resolverRunId ?? runs[0]?.id ?? null; try { const nextSet = await awardsApi.get(activeContest); const nextCandidates = await awardsApi.candidates(activeContest); if (request !== generation || activeContest !== contestId.value) return; awardSet.value = nextSet; candidates.value = nextCandidates; resolverRunId.value = nextSet.resolverRunId; } catch (error) { if (!(error instanceof ApiError && error.status === 404)) throw error; if (request !== generation || activeContest !== contestId.value) return; awardSet.value = null; candidates.value = []; } errorMessage.value = ''; } catch (error) { if (request === generation && activeContest === contestId.value) errorMessage.value = getErrorMessage(error); } finally { if (request === generation) loading.value = false; } }
async function changeContest() { generation += 1; awardSet.value = null; categories.value = []; candidates.value = []; await router.replace({ query: contestId.value ? { contestId: String(contestId.value) } : {} }); await loadWorkspace(); }
function resetForm() { Object.assign(categoryForm, { code: '', name: '', displayOrder: categories.value.length + 1, includeStar: false, groupName: '', participationType: null, firstBlood: false, ruleType: 'FIXED_COUNT', ratio: 0.1, fixedCount: 1, rankFrom: 1, rankTo: 1 }); }
function openCreate() { editing.value = null; resetForm(); categoryVisible.value = true; } function openEdit(category: AwardCategory) { editing.value = category; Object.assign(categoryForm, { code: category.code, name: category.name, displayOrder: category.displayOrder, includeStar: category.includeStar, groupName: category.groupName ?? '', participationType: category.participationType, firstBlood: category.firstBlood, ruleType: category.ruleType, ratio: category.ratio ?? 0.1, fixedCount: category.fixedCount ?? 1, rankFrom: category.rankFrom ?? 1, rankTo: category.rankTo ?? 1 }); categoryVisible.value = true; }
function payload(): AwardCategoryPayload { return { code: categoryForm.code.trim().toUpperCase(), name: categoryForm.name.trim(), displayOrder: categoryForm.displayOrder, includeStar: categoryForm.includeStar, groupName: categoryForm.groupName.trim() || null, participationType: categoryForm.participationType, firstBlood: categoryForm.firstBlood, rule: { ruleType: categoryForm.ruleType, ratio: categoryForm.ruleType === 'RATIO' ? categoryForm.ratio : null, fixedCount: categoryForm.ruleType === 'FIXED_COUNT' ? categoryForm.fixedCount : null, rankFrom: categoryForm.ruleType === 'RANK_RANGE' ? categoryForm.rankFrom : null, rankTo: categoryForm.ruleType === 'RANK_RANGE' ? categoryForm.rankTo : null } }; }
async function saveCategory() { const data = payload(); if (!contestId.value || !data.code || !data.name) { ElMessage.warning('请填写奖项代码和名称'); return; } mutating.value = true; try { const saved = editing.value ? await awardsApi.updateCategory(editing.value.id, editing.value.version, data) : await awardsApi.createCategory(contestId.value, data); categories.value = [...categories.value.filter((item) => item.id !== saved.id), saved].sort((a, b) => a.displayOrder - b.displayOrder); categoryVisible.value = false; ElMessage.success('奖项类别已保存'); } catch (error) { ElMessage.error(getErrorMessage(error)); } finally { mutating.value = false; } }
async function deleteCategory(category: AwardCategory) { try { await ElMessageBox.confirm(`删除“${category.name}”会同时移除该类别的获奖记录，是否继续？`, '删除奖项类别', { type: 'warning' }); } catch { return; } mutating.value = true; try { await awardsApi.deleteCategory(category.id, category.version); await loadWorkspace(); ElMessage.success('奖项类别已删除'); } catch (error) { ElMessage.error(getErrorMessage(error)); } finally { mutating.value = false; } }
async function generateAwards() { if (!contestId.value || !resolverRunId.value) return; try { await ElMessageBox.confirm('重新生成会替换自动生成的名单，但保留手工添加项。是否继续？', '生成奖项名单', { type: 'warning' }); } catch { return; } mutating.value = true; try { awardSet.value = await awardsApi.generate(contestId.value, resolverRunId.value); candidates.value = await awardsApi.candidates(contestId.value); ElMessage.success('奖项名单已生成'); } catch (error) { ElMessage.error(getErrorMessage(error)); } finally { mutating.value = false; } }
async function freezeAwards() { if (!contestId.value || !awardSet.value) return; if (awardSet.value.conflicts.length) { ElMessage.warning('名单仍有重复获奖冲突，请复核后再锁定'); return; } mutating.value = true; try { awardSet.value = await awardsApi.freeze(contestId.value, awardSet.value.version); ElMessage.success('奖项名单已锁定'); } catch (error) { ElMessage.error(getErrorMessage(error)); } finally { mutating.value = false; } } async function unfreezeAwards() { if (!contestId.value || !awardSet.value) return; mutating.value = true; try { awardSet.value = await awardsApi.unfreeze(contestId.value, awardSet.value.version); } catch (error) { ElMessage.error(getErrorMessage(error)); } finally { mutating.value = false; } }
function openManual() { manualCategoryId.value = categories.value[0]?.id ?? null; manualTeamId.value = null; manualVisible.value = true; } async function addRecipient() { if (!contestId.value || !awardSet.value || !manualCategoryId.value || !manualTeamId.value) return; mutating.value = true; try { awardSet.value = await awardsApi.addRecipient(contestId.value, manualCategoryId.value, manualTeamId.value, awardSet.value.version); manualVisible.value = false; } catch (error) { ElMessage.error(getErrorMessage(error)); } finally { mutating.value = false; } } async function removeRecipient(value: unknown) { const recipient = value as AwardRecipient; if (!awardSet.value) return; mutating.value = true; try { awardSet.value = await awardsApi.removeRecipient(recipient.id, awardSet.value.version); } catch (error) { ElMessage.error(getErrorMessage(error)); } finally { mutating.value = false; } }
async function downloadCsv() { if (!contestId.value) return; exporting.value = true; let url: string | null = null; let anchor: HTMLAnchorElement | null = null; try { url = URL.createObjectURL(await awardsApi.csv(contestId.value)); anchor = document.createElement('a'); anchor.href = url; anchor.download = `contest-${contestId.value}-awards.csv`; document.body.append(anchor); anchor.click(); } catch (error) { ElMessage.error(getErrorMessage(error)); } finally { anchor?.remove(); if (url) window.setTimeout(() => URL.revokeObjectURL(url!), 0); exporting.value = false; } }
