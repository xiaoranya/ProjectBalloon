...
const detail = {
  session,
  problems: [{ problemId: 4, slug: 'graph', title: 'Graph', position: 1, solved: false, attempts: 0 }],
};

describe('VirtualPracticeView', () => {
  beforeEach(() => {
    mocks.virtualSessions.mockResolvedValue([session]);
    mocks.problemBank.mockResolvedValue({ content: [{ id: 4, slug: 'graph', title: 'Graph', statement: null, difficulty: 2, tags: [], publishedAt: null }] });
  ...
  it('loads the selected session and archives it only after confirmation', async () => {
    ...
    await wrapper.findAll('button').find((button) => button.text().includes('归档'))!.trigger('click');
    await flushPromises();
