...
use super::{ConfigResponse, audit, require_contest};
...
async fn load_metrics(database: &PgPool, contest: i64) -> Result<PresentationMetrics, AppError> {
    let balloon = sqlx::query_as::<_, (i64,i64,i64,i64,i64,i64,i64)>("SELECT count(*),count(*) FILTER(WHERE is_first_blood),count(*) FILTER(WHERE upper(status)='PENDING'),count(*) FILTER(WHERE upper(status)='PREPARING'),count(*) FILTER(WHERE upper(status)='DELIVERING'),count(*) FILTER(WHERE upper(status)='DELIVERED'),count(*) FILTER(WHERE upper(status)='CANCELLED') FROM balloon_tasks WHERE contest_id=$1").bind(contest).fetch_one(database).await.map_err(|e| AppError::internal("load balloon presentation metrics", e))?;
