...
建设一套支持内网部署的 XCPC/ICPC 竞赛平台，覆盖赛前准备、比赛进行、赛后滚榜、现场运营和数据归档等完整流程。
...
| 超级管理员 | 管理全局配置和所有比赛 |
| 比赛管理员 | 管理指定比赛 |
| 裁判 | 回复 clarification、处理提交和重判 |
| 队伍 | 正式参赛用户 |
| 打星队 | 不同展示标识，但参与气球、滚榜、颁奖 |
| 打印员 | 处理打印任务 |
| 气球工作人员 | 处理气球任务 |
| 滚榜操作员 | 控制 Resolver 滚榜 |
| 颁奖操作员 | 管理获奖名单和颁奖页面 |
| 大屏操作员 | 控制现场大屏 |
| 直播操作员 | 控制直播展示页面 |
...
```text
支持内网部署的 XCPC 竞赛平台，覆盖赛前配置、队伍管理、题目管理、提交判题、实时榜单、封榜、Resolver 滚榜、自动打印、气球配送、颁奖名单生成、多大屏展示和直播辅助。
```
