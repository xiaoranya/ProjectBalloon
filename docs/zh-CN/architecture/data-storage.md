...
全新 Rust 安装从合并后的 alpha 迁移 `migrations/0001_initial.sql` 开始。该文件把原先的时间戳迁移历史合并为单个文件；它不复制先前框架的实体模型。

API 将 SQLx 迁移嵌入其可执行文件，并在接受流量前应用它们。alpha 阶段该合并迁移可进行破坏性编辑；部署应删除并重建（或恢复）数据库，而非原地升级。Rust 版本仅支持全新安装；先前实现的现有安装不得重放该迁移。任何历史数据传输都是独立的导出 / 导入项目，而不是升级保证。
